...
import fauna.tx.consensus.ReplicatedLog
...
trait LogSyntax {
  implicit def asRichLog(log: ReplicatedLog[_]): LogSyntax.RichLog =
    LogSyntax.RichLog(log)
}

object LogSyntax {
  case class RichLog(log: ReplicatedLog[_]) extends AnyVal {
    // This is convenient in tests, but we don't want to encourage it
    // in non-test code; it should just use "ring contains self", otherwise
    // we'd need a somewhat roundabout-looking method "log.isRingMember(ring)".
    def isRingMember =
      log.ring contains log.self
  }
}

package object test extends NetworkHelpers with LogSyntax {
...
package test {
  trait Spec extends AnyFreeSpec
    ...
    private val ramRoot = {
      ...
      Paths.get(base, "fauna-api-test", "tx")
    }
